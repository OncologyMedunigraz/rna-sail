...
#'
#' @importFrom stats median wilcox.test p.adjust
...
run_ssgsea_analysis <- function(
  ...
  pdf(boxplot_file, width = 11, height = 30)

  p <- ggplot(
    scores_plot,
    aes(x = .data$Group, y = .data$score, fill = .data$Group)
  ) +
    ...
    facet_wrap(~ pathway_clean, scales = "free_y") +
